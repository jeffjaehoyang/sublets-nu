...
export const ToogleMainNav = styled.button`
  ...
  span {
    ...
    &:first-child {
      ${(props) => (props.open ? tw`-mb-px` : tw`mb-1`)};
      transform: ${({ open }) => (open ? 'rotate(45deg)' : 'none')};
    }

    &:last-child {
      ${(props) => (props.open ? tw`-mt-px` : tw`mt-1`)};
    ...
    &:nth-child(2) {
      ...
      ${(props) => (props.open ? tw`bg-opacity-0` : tw`bg-opacity-100`)};
      transform: ${(props) => (props.open ? 'translate(20px)' : 'none')};
    }
  }
`;
